...
import (
	...
	"github.com/cronokirby/safenum-misc/internal/randutil"
)
...
func (pub *PublicKey) Equal(x crypto.PublicKey) bool {
	...
	_, eq, _ := pub.N.Cmp(xx.N)
	return eq == 1 && pub.E == xx.E
}
...
func (priv *PrivateKey) Equal(x crypto.PrivateKey) bool {
	...
	if !priv.PublicKey.Equal(&xx.PublicKey) || priv.D.Eq(xx.D) != 1 {
		return false
	}
	...
	for i := range priv.Primes {
		if priv.Primes[i].Eq(xx.Primes[i]) != 1 {
			return false
		}
	}
	return true
...
func (priv *PrivateKey) Validate() error {
	...
	if _, eq, _ := modulus.CmpMod(priv.N); eq != 1 {
...
NextSetOfPrimes:
	for {
		...
		for _, prime := range primes {
			...
			totient.Mul(totient, pminus1, bits)
		}
		...
		ok := priv.D.ModInverse(e, safenum.ModulusFromNat(totient))
...
func decrypt(priv *PrivateKey, c *safenum.Nat) (m *safenum.Nat, err error) {
	if _, _, lt := c.CmpMod(priv.N); lt != 1 {
		err = ErrDecryption
		return
	}
	if priv.N.BitLen() == 0 {
